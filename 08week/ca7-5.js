// https://editor.p5js.org/gs4/sketches/NfI-wJaxu
let velocities = []
let deltaCtrs = []
let deltas = []
let frameRateVals = []
...
function resolveParticleCollisions(idxA, idxB) {
  ...
  if (collisionDist < 0 && dist > 0.0001) {
    ...
    deltas[idxA].x -= random(0.01, -0.01) + (normalDirX * collisionDist) / 2
    deltas[idxA].y -= random(0.01, -0.01) + (normalDirY * collisionDist) / 2
    deltas[idxB].x += random(0.01, -0.01) + (normalDirX * collisionDist) / 2
    deltas[idxB].y += random(0.01, -0.01) + (normalDirY * collisionDist) / 2
    deltaCtrs[idxA] += 1
    deltaCtrs[idxB] += 1
  }
}
...
function draw() {
  ...
  for (let i = 0; i < positions.length; i++) {
    ...
    positions[i].x = oldPositions[i].x + timeStep * velocities[i].x
    positions[i].y =
      oldPositions[i].y +
      timeStep * velocities[i].y +
      gravity * timeStep -
      timeStep * wind
